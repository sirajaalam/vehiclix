...
export default function EditTripPage() {
  ...
  useEffect(() => {
    async function loadInitialData() {
      try {
        const [vData, tripData] = await Promise.all([
          api.listVehicles(),
          api.getTrip(tripId),
        ]);
        ...
        if (tripData) {
          ...
          setVehicleId(tripData.vehicleId || '');
          setStartDate(tripData.startDate ? tripData.startDate.split('T')[0] : '');
          setEndDate(tripData.endDate ? tripData.endDate.split('T')[0] : '');
          setStartLocation(tripData.startLocation || '');
          setDestination(tripData.destination || '');
          setEstimatedDistance(
            tripData.estimatedDistance != null ? tripData.estimatedDistance : ''
          );
          setActualDistance(
            tripData.actualDistance != null ? tripData.actualDistance : ''
          );
          setNotes(tripData.notes || '');
          setTripStatus(tripData.status || 'PLANNED');
          ...
          if (tripData.participants && tripData.participants.length > 0) {
            ...
            setParticipants([{ name: 'Trip Owner' }]);
          }
  ...
  const handleAddExpenseRow = () => {
    setExpenses([
      ...expenses,
      {
        category: 'FUEL',
        amount: '',
        expenseDate: startDate || new Date().toISOString().split('T')[0],
        notes: '',
      },
    ]);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (tripStatus === 'COMPLETED') {
      toast.error('Completed trips are finalized and cannot be modified.');
      return;
    }
    ...
    if (!vehicleId) {
      errs.vehicleId = 'Please select a registered vehicle from your garage';
    }
    if (!startDate) {
    ...
    const payload: any = {
      ...
      endDate: endDate ? safeIsoDate(endDate) : null,
      startLocation: startLocation.trim() || null,
      destination: destination.trim() || null,
      estimatedDistance:
        estimatedDistance !== '' ? Number(estimatedDistance) : null,
      actualDistance: actualDistance !== '' ? Number(actualDistance) : null,
    ...
    try {
      ...
      // Remove stale split cache so new participants load cleanly
      try {
        localStorage.removeItem(`vehiclix_split_${tripId}`);
      } catch (e) {
        // ignore
      }

      router.push(`/trips/${tripId}/split`);
    } catch (err: any) {
      console.error('Failed to update expedition:', err);
      toast.error(err.message || 'Failed to update expedition.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-slate-400">Loading expedition details...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-16">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-display">
                Edit Road Expedition
              </h1>
              <p className="text-xs sm:text-sm text-slate-400">
                Update route details, manage registered participants, and synchronize itemized expenses
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Form Body */}
        <form id="edit-trip-form" onSubmit={handleSave} noValidate className="space-y-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 space-y-6">
              <div className="rounded-2xl border border-white/[0.08] bg-slate-900/60 p-5 sm:p-6 backdrop-blur-xl space-y-5">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1.5">
                    Assigned Garage Vehicle <span className="text-rose-400 font-bold">*</span>
                  </label>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (fieldErrors.estimatedDistance)
                          setFieldErrors((prev) => {
                            const n = { ...prev };
                            delete n.estimatedDistance;
                            return n;
                          });
                      }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (fieldErrors.actualDistance)
                          setFieldErrors((prev) => {
                            const n = { ...prev };
                            delete n.actualDistance;
                            return n;
                          });
                      }}
                      className="w-full h-10 rounded-xl border border-white/[0.1] bg-slate-950/80 px-3.5 text-white text-xs focus:border-emerald-500 focus:outline-none transition-colors font-mono"
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <Modal
                ...
                subtitle="Manage travelers joining this road expedition"
                icon={
              ...
              >
                <div className="space-y-4">
                  <p className="text-xs text-slate-400 leading-relaxed">
                    Update travelers joining this trip. When saved, Splitwise will dynamically recalculate and display rows for all registered travelers.
                  </p>
              ...
              <div className="p-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 space-y-3">
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400">Total Expedition Expenses:</span>
                ...
                <div className="pt-2 border-t border-emerald-500/10 text-[11px] text-slate-400 flex items-center gap-1.5">
                  ...
                  <span>
                    Saving changes will update Splitwise allocations across {participants.length} {participants.length === 1 ? 'traveler' : 'travelers'}.
                  </span>
                </div>
              </div>
            </div>
